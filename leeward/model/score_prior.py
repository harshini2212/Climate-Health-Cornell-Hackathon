...
RUNG = 0
# Re-exported so callers that grew up against score_prior keep working after the
# machinery moved to score.py. One definition, two names.
N_DRAWS = _score.N_DRAWS
DAYS_PER_BUILD = _score.DAYS_PER_BUILD
BLOCK_ROWS = _score.BLOCK_ROWS
ETA_CLIP = _score.ETA_CLIP
LO_Q, HI_Q = _score.LO_Q, _score.HI_Q
MIN_DRIVER_LOGODDS = _score.MIN_DRIVER_LOGODDS
SEED = _score.SEED
...
def score(cohort: pl.DataFrame, hazards: pl.DataFrame, site_status: pl.DataFrame, *,
    ...
    # `seed` governs the missingness marginalisation as well as the coefficient draws,
    # so it has to travel with `coef` -- otherwise this path and `score.score()` draw
    # different unknowns from the same seed and stop being the same numbers.
    return _score.score(cohort, hazards, site_status, dates=dates, coef=coef, seed=seed)
