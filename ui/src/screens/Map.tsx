...
export function Map({ scenario, onSource }: { scenario: string; onSource: (s: Source) => void }) {
  ...
  const layers = useMemo(() => {
    ...
    const overlay = new GeoJsonLayer<Feature["properties"]>({
      ...
      getLineWidth: (f) => {
        const h = hazardByZip.get(f.properties.modzcta);
        if (!h) return 0;
        const on = (showFlood && (h.flood_warning || h.flash_flood_emergency)) || (showOutage && h.outage_frac >= 0.2);
        return on ? 2.5 : 0;
      },
  ...
  const tooltip = (info: PickingInfo) => {
    ...
    if ("facility_id" in o) {
      return {
        html:
          ...
          `<br/>${o.site_down ? "<span style='color:var(--critical)'>SITE DOWN</span>" : "open"}` +
    ...
    const flags = h
      ? [
          h.heat_alert ? "heat alert" : null,
          h.smoke_alert ? "smoke" : null,
          h.flood_warning ? "flood warning" : null,
          h.flash_flood_emergency ? "flash flood emergency" : null,
          h.outage_frac >= 0.2 ? `${Math.round(h.outage_frac * 100)}% outage` : null,
          h.mail_delivery_disrupted ? "mail disrupted" : null,
        ].filter(Boolean)
      : [];
    return {
      html:
        ...
        (s
          ? `<br/>${NEED_LABEL[need]}: <strong>${s.expected_count.toFixed(1)}</strong> expected (${s.lo80.toFixed(1)}–${s.hi80.toFixed(1)}) of ${s.n_panel} on the panel`
          : "<br/>no veterans on the panel here") +
        (h ? `<br/>heat index ${h.heat_index_max_f.toFixed(0)}°F · PM2.5 ${h.pm25.toFixed(0)}` : "") +
        (flags.length ? `<br/><span style='color:var(--serious)'>${flags.join(" · ")}</span>` : "") +
        `</div>`,
      style: { background: "none", padding: "0" },
    };
  };
  ...
  return (
    <div className="screen">
      <aside className="panel">
        <h2>Expected need by ZIP</h2>
        {forecast?.headline && <div className="headline">{forecast.headline}</div>}
        {error && <div className="headline">{error}</div>}

        <div className="field">
          <h3>Need</h3>
          <select value={need} onChange={(e) => setNeed(e.target.value)} aria-label="Need">
            {NEEDS.map((n) => (
        ...
        <div className="field">
          ...
          <div className="row" style={{ justifyContent: "space-between" }}>
            <span>{date ?? "—"}</span>
            <span style={{ color: "var(--muted)" }}>day {dayIdx + 1} of {forecast?.dates.length ?? 7}</span>
          </div>
          <input
            type="range"
            min={0}
            max={Math.max(0, (forecast?.dates.length ?? 1) - 1)}
            value={dayIdx}
        ...
        <div>
          <h3>{NEED_LABEL[need]}, expected count</h3>
          <div className="legend-ramp">
            {SEQUENTIAL_HEX.map((h) => (
              <span key={h} style={{ background: h }} />
            ))}
          </div>
          <div className="legend-ends">
            <span>0</span>
            <span>{maxCount.toFixed(1)}</span>
          </div>
          {scores && <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>model rung {scores.model_rung}</div>}
        </div>
        ...
        <div>
          ...
          <label className="check">
            ...
            <span className="swatch" style={{ background: STATUS_HEX.critical }} /> Flood warning or surge
          </label>
          <label className="check">
            <input type="checkbox" checked={showOutage} onChange={(e) => setShowOutage(e.target.checked)} />
            <span className="swatch" style={{ background: STATUS_HEX.serious }} /> Power outage ≥ 20%
          </label>
        </div>
        ...
        <div>
          ...
          <div className="legend-item">
            <span className="swatch pin" style={{ background: "#2a78d6" }} /> open
          </div>
          <div className="legend-item" style={{ marginBottom: 8 }}>
            <span className="swatch pin" style={{ background: STATUS_HEX.critical }} /> site down
          </div>
          <div className="facilities">
            {majors.map((f) => (
              <div key={f.facility_id} className={`facility${f.site_down ? " down" : ""}`}>
                <span className="name" title={f.name}>
                  {f.facility_id} · {f.name}
                </span>
                <span className={f.site_down ? "status-down" : "status-open"}>{f.site_down ? "DOWN" : `zone ${f.evac_zone}`}</span>
              </div>
            ))}
          </div>
          {down.length === 0 && <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 6 }}>All 14 sites open on this day.</div>}
        </div>
      </aside>
